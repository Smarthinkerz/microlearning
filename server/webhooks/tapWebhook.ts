...
function verifyTapSignature(
  ...
  if (!ENV.tapWebhookSecret) {
    ...
    return !ENV.isProduction; // Only skip in dev
  }
  if (!signature) {
...
async function handleChargeCaptured(payload: TapWebhookPayload): Promise<void> {
  ...
  if (sub) {
    ...
    if (payload.customer?.id) {
      await db.updateSubscriptionExternalIds(sub.id, payload.id, payload.customer.id);
    }
  }
...
async function handleChargeFailed(payload: TapWebhookPayload): Promise<void> {
  ...
  // Keep subscription in pending/trial state
  const orgId = payment.orgId;
  const sub = await db.getOrgSubscription(orgId);
  if (sub && sub.status === "active") {
    // If it was already active (renewal failure), mark as past_due
    await db.updateSubscriptionStatus(sub.id, "past_due" as any);
  }
...
async function handleRefund(payload: TapWebhookPayload): Promise<void> {
  ...
  const sub = await db.getOrgSubscription(orgId);
  if (sub) {
    await db.updateSubscriptionStatus(sub.id, "canceled", Date.now());
  }
  ...
  await db.createAuditLog({
    ...
    details: {
      chargeId: payload.id,
      refundAmount: payload.refund?.amount ?? payload.amount,
      orgId,
    } as any,
  });
...
/**
 * Express handler that receives raw body for signature verification.
 * Tap sends webhooks as JSON with a hashstring header.
 */
...
/**
 * Determine the event type from the Tap payload.
 * Tap doesn't always send an explicit event type — we infer from status.
 */
